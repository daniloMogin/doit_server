        ...
        async (req: Request, res: Response) => {
            ...
            if (token) {
                ...
                try {
                    ...
                    res.status(500).json({
                        success: false, msg: 'Get all users error ' + error
                    });
                }
            } else {
                return res
            ...
            if (token) {
                try {
                    ...
                    if (findUserByUsername != null) {
                        res.status(200).json({ success: true, user: findUserByUsername });
                    } else {
                        res.status(500).json({ success: false, msg: findUserByUsername });
                    }
                } catch (error) {
                    res.status(500).json({
                        success: false, msg: error
            ...
            if (token) {
                ...
                try {
                    ...
                    if (createUser.errmsg === undefined) {
                        ...
                        res.status(500).json({ success: false, msg: createUser });
                    }
                } catch (error) {
                    res.status(500).json({ success: false, msg: 'Create user error ' + error });
                }
            } else {
                return res
                    .status(403)
        ...
        try {
            ...
            if (_.isNil(validate_login.error)) {
                ...
                if (!_.isNil(authenticate_user_email)) {
                    ...
                    res.status(201).json({ success: true, msg: authenticate_user_password });
                } else {
                    res.status(500).json({ success: false, msg: authenticate_user_email });
                }
            } else {
        ...
        try {
            ...
            if (findUserByUsername != null) {
                ...
                if (_.isNil(validate_register.error)) {
                    ...
                    res.status(500).json({ success: false, msg: validate_register });
                }
            }
        } catch (error) {
            res.status(500).json({ success: false, msg: 'Register user error ' + error });
        }
    }
